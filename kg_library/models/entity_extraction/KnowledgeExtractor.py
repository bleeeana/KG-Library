...
class KnowledgeGraphExtractor:
    ...
    def __extract_triplets_typed(self, text: str):
        ...
        for token in text.replace("<s>", "").replace("<pad>", "").replace("</s>", "").replace("tp_XX", "").replace(
            ...
            if token == "<triplet>" or token == "<relation>":
                ...
                if relation:
                    triplets.append({
                        'head': subject.strip(), 'head_type': subject_type,
                        'type': relation.strip(), 'tail': object_.strip(), 'tail_type': object_type
                    })
                    relation = ''
                subject = ''
    ...
    def extract_from_text(self, text: str, num_beams=2, max_length=1024) -> list:
        ...
        for sentence in decoded_preds:
            ...
            for t in triplets:
                self.triplets.add((t['head'], t['type'], t['tail']))  # для GNN
        return all_triplets
